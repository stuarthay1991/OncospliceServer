		...
		try{
            ...
            const signaturePromises = selectAllSignaturesResult.rows.map(async row => {
                ...
                const annotationPromises = annotation_names.map(async annotation => {
                    var annotationQuery = "";
                    annotationQuery = "SELECT COUNT(eventannotation) FROM ".concat(postedCancer).concat("_fullsig WHERE signature_name = '").concat(current_signature).concat("' AND eventannotation LIKE '").concat(annotation).concat("'");
